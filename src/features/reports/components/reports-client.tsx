    ...
    const handleSync = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                toast.success(msg);
                const updatedSummary = await getSalesSummary() as any;
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
                <Card className="bg-gradient-to-br from-emerald-600 to-emerald-800 text-white border-none shadow-xl shadow-emerald-100 p-2">
                    ...
                    <CardContent>
                        <div className="text-4xl font-black tracking-tighter">{formatCurrency(workingCapital, currency)}</div>
                ...
                <Card className="bg-gradient-to-br from-blue-600 to-blue-800 text-white border-none shadow-xl shadow-blue-100 p-2">
                    ...
                    <CardContent>
                        <div className="text-4xl font-black tracking-tighter">{formatCurrency(summary?.cashBalance || 0, currency)}</div>
                ...
                <Card className="bg-gradient-to-br from-slate-700 to-slate-900 text-white border-none shadow-xl shadow-slate-200 p-2">
                    ...
                    <CardContent>
                        <div className="text-4xl font-black tracking-tighter">{formatCurrency(summary?.inventoryValue || 0, currency)}</div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-3">
                <Card className="bg-white border shadow-sm group hover:border-blue-200 transition-colors">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-bold text-slate-500">{dict.Reports.Overview.Today}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-black text-slate-900">{formatCurrency(summary?.daily?.total || 0, currency)}</div>
                        <div className="text-xs font-bold text-blue-600 mt-1">{summary?.daily?.count || 0} {dict.Reports.Overview.Invoice}</div>
                    </CardContent>
                </Card>

                <Card className="bg-white border shadow-sm group hover:border-indigo-200 transition-colors">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-bold text-slate-500">{dict.Reports.Overview.Month}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-black text-slate-900">{formatCurrency(summary?.monthly?.total || 0, currency)}</div>
                        <div className="text-xs font-bold text-indigo-600 mt-1">{summary?.monthly?.count || 0} {dict.Reports.Overview.Deals}</div>
                    </CardContent>
                </Card>

                <Card className="bg-white border shadow-sm group hover:border-emerald-200 transition-colors">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-bold text-slate-500">{dict.Reports.Overview.YearlySales}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="text-2xl font-black text-slate-900">{formatCurrency(summary?.yearly?.total || 0, currency)}</div>
            ...
            <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
                ...
                <Link href="/dashboard/reports/department-sales" className="bg-teal-50 hover:bg-teal-100 p-5 rounded-2xl flex flex-col items-center justify-center text-center transition-all border border-teal-200 group">
