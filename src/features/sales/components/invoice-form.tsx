...
export function InvoiceForm({ products, customers, representatives = [], settings }: {
    ...
    const { control, register, handleSubmit, watch, setValue, reset, formState: { errors, isSubmitting } } = useForm<z.infer<typeof invoiceSchema>>({
        ...
        defaultValues: {
            customerName: dict.POS?.CashCustomer || "Cash Customer",
            customerId: "0",
    ...
    const handleBarcodeScan = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
            ...
            if (product) {
                ...
                toast.success((dict.Common?.Success || "Success") + ": " + product.name);
    ...
    const onSubmit = async (data: z.infer<typeof invoiceSchema>) => {
        try {
            ...
            if (res && res.success) {
                toast.success(res.message || (dict.Common?.Success || "Success"));
                if (res.id) {
                ...
                setTimeout(() => {
                    reset({
                        customerName: dict.Sales?.Invoice?.Form?.CashCustomer || "Cash Customer",
                ...
                toast.error(res?.message || (dict.Common?.Error || "Error"));
            }
        } catch (e: any) {
                ...
                <CardContent className="p-4 flex items-center gap-4">
                    <div className="flex-1">
                        <label className="text-sm font-semibold text-blue-700 mb-1 block">
                            🔍 {dict.Sales?.Invoice?.Form?.Table?.Item || "Scan/Search"} (Enter)
                        </label>
                        <Input autoFocus placeholder={dict.Sales?.Invoice?.Form?.SelectProduct || "Select Product"} onKeyDown={handleBarcodeScan} className="bg-white" />
                    </div>
                </CardContent>
            </Card>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" dir={dir}>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 border p-4 rounded-md bg-gray-50/50">
                    <div className="md:col-span-2">
                        <label className="text-sm font-medium mb-1 block">{dict.Sales?.Invoice?.Form?.Customer || "Customer"}</label>
                        <CustomerCombobox
                            customers={customers}
                            ...
                            onSelect={(val) => {
                                ...
                                else if (val === "0") setValue("customerName", dict.Sales?.Invoice?.Form?.CashCustomer || "Cash Customer");
                            }}
                            placeholder={dict.Sales?.Invoice?.Form?.SelectCustomer || "Select Customer"}
                        />
                        {errors.customerName && <p className="text-red-500 text-xs mt-1">{errors.customerName.message}</p>}
                    </div>
                    <div>
                        <label className="text-sm font-medium mb-1 block">{dict.Sales?.Invoice?.Form?.IssueDate || "Date"}</label>
                        <Input type="date" {...register("issueDate")} />
                    </div>

                    <div>
                        <label className="text-sm font-medium mb-1 block">{dict.Representatives?.MenuLabel || "Representative"}</label>
                        <Select onValueChange={(val) => setValue("representativeId", parseInt(val))} defaultValue={watch("representativeId")?.toString()}>
                            <SelectTrigger className="bg-white">
                                <SelectValue placeholder={dict.Representatives?.SearchPlaceholder || "Select"} />
                            </SelectTrigger>
                            <SelectContent>
                    ...
                    <div>
                        <label className="text-sm font-medium mb-1 block">{dict.Sales?.Invoice?.Form?.Currency || "Currency"}</label>
                        <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" {...register("currency")}>
                            <option value="EGP">{dict.Settings?.Form?.Currencies?.EGP || "EGP"}</option>
                            <option value="USD">{dict.Settings?.Form?.Currencies?.USD || "USD"}</option>
                            <option value="EUR">{dict.Settings?.Form?.Currencies?.EUR || "EUR"}</option>
                            <option value="SAR">{dict.Settings?.Form?.Currencies?.SAR || "SAR"}</option>
                        </select>
                    </div>
                    {watch("currency") !== 'EGP' && (
                        <div>
                            <label className="text-sm font-medium mb-1 block">{dict.Sales?.Invoice?.Form?.ExchangeRate || "Exchange Rate"}</label>
                    ...
                    {settings?.taxEnabled && (
                        <div className="flex flex-col gap-2 pt-2">
                            <div className="flex items-center gap-2">
                                ...
                                <label htmlFor="includeTax" className="text-sm font-medium">{dict.Sales?.Invoice?.Form?.IncludeTax || "Include Tax"}</label>
                            </div>
                        </div>
                    )}
                    <div className="flex flex-col gap-2 pt-6">
                        <div className="flex items-center gap-2">
                            ...
                            <label htmlFor="isInstallment" className="text-sm font-bold text-orange-600">💳 {dict.Sales?.Invoice?.Form?.IsInstallment || "Installment"}</label>
                        </div>
                    </div>
                </div>
                ...
                {isInstallment && (
                    <div className="bg-orange-50 border border-orange-200 p-4 rounded-md grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label className="text-sm font-bold text-orange-800 mb-1 block">{dict.Sales?.Invoice?.Form?.InstallmentMonths || "Months"}</label>
                            <Input type="number" {...register("installmentCount", { valueAsNumber: true })} min="1" />
                        </div>
                        <div>
                            <label className="text-sm font-bold text-orange-800 mb-1 block">{dict.Sales?.Invoice?.Form?.InstallmentInterest || "Interest %"}</label>
                        ...
                        <div className="bg-white p-2 rounded border border-orange-200 flex flex-col items-center justify-center">
                            <span className="text-xs text-orange-600 font-bold">{dict.Sales?.Invoice?.Form?.ExpectedMonthlyInstallment || "Monthly"}</span>
                ...
                <Card>
                    <Table>
                        <TableHeader>
                            <TableRow>
                                <TableHead className="w-[30%]">{dict.Sales?.Invoice?.Form?.Table?.Item || "Item"}</TableHead>
                                <TableHead className="w-[30%]">{dict.Sales?.Invoice?.Form?.Table?.Description || "Description"}</TableHead>
                                <TableHead className="w-[10%]">{dict.Sales?.Invoice?.Form?.Table?.Qty || "Qty"}</TableHead>
                                <TableHead className="w-[15%]">{dict.Sales?.Invoice?.Form?.Table?.Price || "Price"}</TableHead>
                                <TableHead className="w-[10%]">{dict.Sales?.Invoice?.Form?.Table?.Total || "Total"}</TableHead>
                                ...
                                <TableRow key={field.id}>
                                    <TableCell className="p-2">
                                        <ProductCombobox
                                            ...
                                            placeholder={dict.Sales?.Invoice?.Form?.SelectProduct || "Search"}
                                        />
                                    </TableCell>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                    <Button type="button" variant="outline" onClick={() => append({ productId: "", description: "", quantity: 1, unitPrice: 0 })} className="gap-2">
                        <Plus size={16} /><span>{dict.Sales?.Invoice?.Form?.AddItem || "Add Item"}</span>
                    </Button>
                    <div className="w-full md:w-80 space-y-4">
                        <div className="bg-gray-50 p-4 rounded-lg border space-y-2">
                            <div className="flex justify-between text-sm"><span>{dict.Sales?.Invoice?.Form?.Subtotal || "Subtotal"}:</span><span className="font-mono">{formatCurrency(totals.subtotal, watch("currency"))}</span></div>
                            {discountAmount > 0 && <div className="flex justify-between text-sm text-red-600"><span>{dict.Sales?.Invoice?.Form?.Discount || "Discount"}:</span><span className="font-mono">-{formatCurrency(discountAmount, watch("currency"))}</span></div>}
                            <div className="flex justify-between text-lg font-bold border-t pt-2 mt-2"><span>{dict.Sales?.Invoice?.Form?.GrandTotal || "Total"}:</span><span className="text-primary font-mono text-xl">{formatCurrency(totals.total, watch("currency"))}</span></div>
                        </div>
                        <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 shadow-sm relative overflow-hidden">
                            {(totals.total - initialPayment) > 0.1 && !isInstallment && (
                                <div className="absolute top-0 right-0 bg-red-600 text-white text-[10px] font-black px-4 py-1 rounded-bl-xl">{dict.Sales?.Invoice?.Form?.CreditSaleTag || "⚠️ Credit Sale"}</div>
                            )}
                            <label className="text-sm font-bold text-blue-900 mb-1 block">{dict.Sales?.Invoice?.Form?.PaidAmount || "Paid"}</label>
                        ...
                        {(totals.total - initialPayment) > 0.1 && (
                            <Card className="bg-rose-50 border-rose-200">
                                <CardContent className="p-4 space-y-3">
                                    <label className="text-sm font-bold text-amber-900 block">📅 {dict.Sales?.Invoice?.Form?.DueDate || "Due Date"}</label>
                ...
                <div className="flex justify-end items-center gap-6">
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] font-bold text-muted-foreground uppercase text-center block mb-1">{dict.Sales?.Invoice?.Form?.PrintLayout || "Layout"}</label>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="auto-print-invoice" className="font-bold text-slate-600 cursor-pointer">{dict.Sales?.Invoice?.Form?.AutoPrint || "Auto Print"}</Label>
                    </div>
                    <Button type="submit" size="lg" className="gap-2 min-w-[150px]" disabled={isSubmitting}><Save size={18} /><span>{dict.Sales?.Invoice?.Form?.Submit || "Save"} (F2)</span></Button>
                </div>
