    ...
    return (
        <div ref={ref} className="p-2 text-black font-mono text-[12px] leading-tight w-[80mm] mx-auto">
            <div className="text-center mb-4">
                ...
                <p>{dict.POS.ReceiptWelcome}</p>
            </div>

            <div className="mb-2 space-y-1">
                <div className="flex justify-between">
                    <span>{dict.Sales.Invoice.Form.IssueDate}:</span>
                    <span>{new Date(data.date).toLocaleDateString(lang === 'ar' ? 'ar-EG' : 'en-GB')}</span>
                </div>
                <div className="flex justify-between">
                    <span>{dict.POS.ReceiptTime}:</span>
                    <span>{new Date(data.date).toLocaleTimeString(lang === 'ar' ? 'ar-EG' : 'en-US', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <div className="flex justify-between">
                    <span>{dict.Sales.Invoice.Form.Table.Number}:</span>
                    <span>{data.invoiceNumber || '---'}</span>
                </div>
                <div className="flex justify-between">
                    <span>{dict.Sales.Invoice.Form.Customer}:</span>
                ...
                {data.tokenNumber && (
                    <div className="flex flex-col items-center border-2 border-black border-dashed mt-2 p-1">
                        <span className="text-[10px] font-bold uppercase">{dict.POS.QueueNo}</span>
            ...
            <div className="border-t border-b border-black border-dashed py-1 mb-2">
                <div className="flex font-bold mb-1">
                    <span className="w-8">{dict.Sales.Invoice.Form.Table.Qty}</span>
                    <span className="flex-1">{dict.Sales.Invoice.Form.Table.Item}</span>
                    <span className="w-12 text-right">{dict.Sales.Invoice.Form.Table.Price}</span>
                </div>
            ...
            <div className="mb-4 space-y-1 font-bold">
                ...
                {data.discount !== undefined && data.discount > 0 && (
                    <div className="flex justify-between text-red-700">
                        <span>{dict.POS.Discount}:</span>
                        <span>-{data.discount.toFixed(2)}</span>
                    </div>
                )}
                {data.paymentMethod && (
                    <div className="flex justify-between text-[10px]">
                        <span>{dict.POS.PaymentMethod}:</span>
                ...
                <div className="flex justify-between text-lg border-t-2 border-black pt-1 mt-1">
                    ...
                    <span>{data.total.toFixed(2)} {currency}</span>
                </div>
            </div>

            <div className="text-center text-[10px] mt-4">
                <p>{dict.POS.ReceiptFooter1}</p>
                <p>{dict.POS.ReceiptFooter2}</p>
                <p className="mt-2">*** {dict.POS.PoweredBy} ***</p>
            </div>
        </div>
    );
});
