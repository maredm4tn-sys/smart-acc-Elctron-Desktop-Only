...
export function BulkImportEmployeesDialog() {
    ...
    const processData = async (jsonData: any[]) => {
        ...
        const result = await bulkImportEmployees(jsonData);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="bg-white hover:bg-slate-50 border-slate-200 text-slate-700 font-bold gap-2 shadow-sm rounded-xl h-10 px-4">
                    ...
                    <span className="hidden sm:inline">{dict.Employees?.ImportExcel}</span>
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>{dict.Employees?.ImportExcel}</DialogTitle>
                    <DialogDescription>
                        {dict.Employees?.Description}
                    </DialogDescription>
                </DialogHeader>
