        ...
        <Select value={value?.toString()} onValueChange={(v) => onChange(Number(v))}>
            ...
            <SelectContent>
                {list.length > 0 ? list.map((item) => (
                    ...
                    <div className="p-2 text-sm text-gray-500 text-center">{dict.Common.NA}</div>
                )}
            </SelectContent>
        </Select>
    );
...
export function VoucherForm({ customers, suppliers, defaultType = 'receipt' }: { customers: any[], suppliers: any[], defaultType?: 'receipt' | 'payment' }) {
    ...
    const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<VoucherFormValues>({
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        if (values.partyType !== 'other' && !values.partyId) {
            toast.error(dict.Vouchers.Form.SelectParty);
            return;
        }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" dir={dir}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    <Label className="text-start block w-full">{dict.Vouchers?.Form?.Type}</Label>
                    <Select onValueChange={(val) => setValue('type', val as any)} defaultValue={type}>
                        <SelectTrigger>
                            <SelectValue placeholder={dict.Vouchers.Form.Placeholder.SelectType} />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div className="space-y-2">
                    <Label className="text-start block w-full">{dict.Vouchers?.Form?.Date}</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    <Label className="text-start block w-full">{dict.Vouchers?.Form?.PartyType}</Label>
                ...
                {partyType === 'customer' && (
                    <div className="space-y-2">
                        <Label className="text-start block w-full">{dict.Vouchers?.Form?.Party}</Label>
                ...
                {partyType === 'other' && (
                    <div className="space-y-2">
                        <Label className="text-start block w-full">{dict.Vouchers.Form.Account}</Label>
                        ...
                        />
                        {errors.accountId && <p className="text-sm text-red-500">{dict.Common.Error}</p>}
                    </div>
                )}
            </div>

            <div className="space-y-2">
                <Label className="text-start block w-full">{dict.Vouchers?.Form?.Amount}</Label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                    <Label className="text-start block w-full">{dict.Vouchers?.Form?.Reference}</Label>
                    <Input {...register('reference')} placeholder={dict.Vouchers.Form.Placeholder.Reference} />
                </div>

                <div className="space-y-2">
                    <Label className="text-start block w-full">{dict.Vouchers?.Form?.Description}</Label>
