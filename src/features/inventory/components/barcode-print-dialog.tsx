...
export function BarcodePrintDialog({ product, open: externalOpen, onOpenChange: externalOnOpenChange }: { product: Product, open?: boolean, onOpenChange?: (open: boolean) => void }) {
    ...
    const SingleLabel = () => (
        <div
            className="print-label relative flex flex-col items-center justify-center bg-white overflow-hidden"
            style={{
        ...
        >
            {settings.showCompanyName && (
                <div
                    className="font-bold text-center w-full whitespace-nowrap overflow-hidden text-ellipsis"
            ...
            {settings.showProductName && (
                <div
                    className="text-center w-full px-1 whitespace-nowrap overflow-hidden text-ellipsis"
            ...
            <div className="flex justify-between w-full px-1 items-end">
                {settings.showSKU && <span className="font-mono text-[10px]">{product.sku}</span>}
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            {externalOpen === undefined && (
                <DialogTrigger asChild>
                    <Button variant="ghost" size="icon" title={dict.Inventory.Barcode.Print}>
            ...
            <DialogContent className="sm:max-w-[900px]">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        {dict.Inventory.Barcode.Title} - {product?.name || dict.Inventory.ProductForm.NoName}
                    </DialogTitle>
                    <DialogDescription>{dict.Inventory.Barcode.Description}</DialogDescription>
                </DialogHeader>

                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 py-4">
                    {/* Settings Panel */}
                    <div className="md:col-span-4 space-y-4 border-l pl-4 rtl:border-l-0 rtl:border-r rtl:pr-4 text-right">
                        <div className="space-y-2">
                            <Label>{dict.Inventory.Barcode.CompanyName}</Label>
                            <Input
                        ...
                        <div className="space-y-4 pt-2">
                            <div className="flex items-center gap-2">
                                ...
                                />
                                <Label htmlFor="showCompany">{dict.Inventory.Barcode.ShowCompany}</Label>
                            </div>
                            <div className="flex items-center gap-2">
                                <Checkbox
                                    id="showProduct"
                                ...
                                />
                                <Label htmlFor="showProduct">{dict.Inventory.Barcode.ShowProduct}</Label>
                            </div>
                            <div className="flex items-center gap-2">
                                <Checkbox
                                    id="showPrice"
                                ...
                                />
                                <Label htmlFor="showPrice">{dict.Inventory.Barcode.ShowPrice}</Label>
                            </div>
                        </div>
                        ...
                        <div className="pt-4">
                            <Button onClick={() => handlePrint && handlePrint()} className="w-full gap-2" size="lg">
                                <Printer className="h-4 w-4" /> {dict.Inventory.Barcode.Print}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-8 bg-slate-100 rounded-lg p-8 flex flex-col items-center justify-center min-h-[400px]">
                        <h3 className="text-sm font-semibold text-slate-500 mb-4">{dict.Inventory.Barcode.Preview}</h3>
