...
import { bulkImportRepresentatives } from "../actions";
import { useTranslation } from "@/components/providers/i18n-provider";

export function BulkImportRepresentativesDialog() {
    ...
    const processData = async (jsonData: any[]) => {
        ...
        const result = await bulkImportRepresentatives(jsonData);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="bg-white hover:bg-slate-50 border-slate-200 text-slate-700 font-bold gap-2 shadow-sm rounded-xl h-10 px-4">
                    ...
                    <span className="hidden sm:inline">{dict.Representatives?.ImportExcel}</span>
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>{dict.Representatives?.ImportExcel}</DialogTitle>
                    <DialogDescription>
                        {dict.Representatives?.Description}
                    </DialogDescription>
                </DialogHeader>
