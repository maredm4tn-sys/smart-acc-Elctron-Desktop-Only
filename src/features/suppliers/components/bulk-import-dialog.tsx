...
export function BulkImportSuppliersDialog() {
    ...
    const processData = async (jsonData: any[]) => {
        ...
        const result = await bulkImportSuppliers(jsonData);

        if (result.success) {
            toast.success(result.message || dict.Common?.Import?.Success);
            setOpen(false);
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="bg-white hover:bg-slate-50 border-slate-200 text-slate-700 font-bold gap-2 shadow-sm rounded-xl h-10 px-4">
                    ...
                    <span className="hidden sm:inline">{dict.Suppliers?.ImportExcel}</span>
                </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>{dict.Suppliers?.ImportExcel}</DialogTitle>
                    <DialogDescription>
                        {dict.Suppliers?.Description}
                    </DialogDescription>
                </DialogHeader>
