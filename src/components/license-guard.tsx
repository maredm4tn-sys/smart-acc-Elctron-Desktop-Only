...
export function LicenseGuard({ children }: { children: React.ReactNode }) {
    ...
    const [isLicensed, setIsLicensed] = useState<boolean | null>(true); // DEFAULT TRUE to bypass loading
    ...
    const handleActivate = async () => {
        ...
        if (isValid) {
            ...
            setIsLicensed(true);
            toast.success(dict.Common.License.SuccessToast);
        } else {
            toast.error(dict.Common.License.ErrorToast);
        }
    };
        ...
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4" dir={dir}>
            <Card className="w-full max-w-md border-red-500/50 shadow-2xl bg-white/95 backdrop-blur">
                <CardHeader className="text-center">
                    ...
                    <CardTitle className="text-2xl font-bold text-slate-800">{dict.Common.License.Unactivated}</CardTitle>
                    <p className="text-sm text-slate-500 mt-2">
                        {dict.Common.License.SingleDeviceHint}
                    </p>
                </CardHeader>
                <CardContent className="space-y-6">
                    <div className="bg-slate-100 p-4 rounded-lg text-center border border-slate-200">
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-wider mb-1">{dict.Common.HardwareId} (Machine ID)</p>
                        <code className="text-lg font-mono font-bold text-slate-900 select-all">{machineId || dict.Common.License.Reading}</code>
                    </div>

                    <div className="space-y-2">
                        <label className="text-sm font-medium">{dict.Common.EnterActivationKey} (Activation Key)</label>
                        <Input
