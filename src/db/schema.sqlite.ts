...
export const tenants = sqliteTable('tenants', {
    ...
    autoBackupTime: text('auto_backup_time').default('00:00'),
    autoBackupTimes: text('auto_backup_times').default('["00:00"]'),
    autoBackupPath: text('auto_backup_path'),
    lastAutoBackupDate: text('last_auto_backup_date'),
    lastAutoBackupTimesDone: text('last_auto_backup_times_done'), // Track which times were done today
    smtpHost: text('smtp_host'),
    smtpPort: integer('smtp_port'),
    smtpUser: text('smtp_user'),
    smtpPass: text('smtp_pass'),
    backupRecipientEmail: text('backup_recipient_email'),
    emailBackupEnabled: integer('email_backup_enabled', { mode: 'boolean' }).default(false),
    telegramBotToken: text('telegram_bot_token'),
    telegramChatId: text('telegram_chat_id'),
    telegramBackupEnabled: integer('telegram_backup_enabled', { mode: 'boolean' }).default(false),

    // --- Settings Columns ---
    defaultPrintSales: text('default_print_sales').default('standard'),
    defaultPrintPOS: text('default_print_pos').default('thermal'),

    taxEnabled: integer('tax_enabled', { mode: 'boolean' }).default(false),
    taxName: text('tax_name').default('ضريبة القيمة المضافة'),
    taxNameEn: text('tax_name_en').default('VAT'),
    taxRate: real('tax_rate').default(14.0),
    ...
    numeralSystem: text('numeral_system').default('latn').notNull(), // 'latn' (123) or 'arab' (١٢٣)
    zoomLevel: real('zoom_level').default(1.0).notNull(),

    createdAt: integer('created_at', { mode: 'timestamp' }).default(sql`(unixepoch())`).notNull(),
    updatedAt: integer('updated_at', { mode: 'timestamp' }).default(sql`(unixepoch())`).notNull(),
});

// --- Users (Renamed to app_users to force fresh schema) ---
export const users = sqliteTable('app_users', {
    ...
    role: text('role').default('cashier').notNull(), // admin, cashier, salesperson, storekeeper, etc.
...
export const representatives = sqliteTable('representatives', {
    ...
    commissionBasis: text('commission_basis').default('sale'), // 'sale', 'collection', 'both'
    notes: text('notes'),
    isActive: integer('is_active', { mode: 'boolean' }).default(true).notNull(),
    createdAt: integer('created_at', { mode: 'timestamp' }).default(sql`(unixepoch())`),
});
...
export const partners = sqliteTable('partners', {
    ...
    sharePercentage: text('share_percentage').default('0.00'), // احتساب آلي
    currentBalance: text('current_balance').default('0.00').notNull(), // جاري الشريك (مدين/دائن)
    isActive: integer('is_active', { mode: 'boolean' }).default(true).notNull(),
    createdAt: integer('created_at', { mode: 'timestamp' }).default(sql`(unixepoch())`),
});
...
// --- Partner Transactions (عمليات الشركاء) ---
export const partnerTransactions = sqliteTable('partner_transactions', {
    ...
    type: text('type').notNull(), // 'capital_increase', 'withdrawal_cash', 'withdrawal_goods', 'profit_share', 'settlement'
    amount: text('amount').notNull(),
    date: text('date').notNull(), // ISO Date
    description: text('description'),

    // For goods withdrawal
    productId: integer('product_id').references(() => products.id),
...
export const partnerTransactionsRelations = relations(partnerTransactions, ({ one }) => ({
    tenant: one(tenants, {
        fields: [partnerTransactions.tenantId],
        references: [tenants.id],
    }),
