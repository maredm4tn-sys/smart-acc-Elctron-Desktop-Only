...
export async function GET() {
    try {
        ...
        try {
            await db.run(sql`ALTER TABLE users ALTER COLUMN email DROP NOT NULL;`);
        } catch (e) {
        ...
        try {
            ...
            await db.run(sql`UPDATE users SET username = email WHERE username IS NULL;`);
        } catch (e) {
        ...
        try {
            await db.run(sql`UPDATE users SET role = 'cashier' WHERE role NOT IN ('admin', 'cashier');`);
        } catch (e) {
